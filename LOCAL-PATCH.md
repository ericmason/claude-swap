...
Branch `fix/wrong-account-and-resume-logout` on the fork `git@github.com:ericmason/claude-swap.git`, on top of tag `v0.26.0`. The first commit is the recovered patch: six files, 1426 insertions, 16 deletions. Two later commits apply what two rounds of Codex review found, the second of which replaced the `ps` parsing with the structured kernel sources.
